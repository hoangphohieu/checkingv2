...
class InputExcel extends Component {
    ...
    convertBarcode = (barcode) => {
        ...
        let check = barcode.toString();
        if (check.split("").length !== 14) this.alertError("co loi upload.js - barcode");
    ...
    convertData = (data) => {
        ...
        dataObj.map((param, key) => { // lọc date, country, và id
            ...
            param["datatype"] = "item";
            param["barcode"] = this.convertBarcode(Date.parse(new Date()) * 10 + key);
        ...
        if (alertObj.length !== 0) {
            ...
            this.alertError("Kiem tra  aaaa" + alertObj.map(param => param.name));
